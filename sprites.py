...
# [BOTS] freeplay crew AI tuning
BOT_SPEED = 240          # cruising speed (player moves at PLAYER_SPEED = 400)
BOT_RUN_SPEED = 330      # fleeing a body / rushing to fix a sabotage
BOT_FIX_RADIUS = 90      # close enough to the panel to start fixing
BOT_LEASH = 360          # soft: past this a bot steers back toward its spawn
BOT_HARD_LIMIT = 470     # hard: a bot's position is clamped to never exceed this
                         # distance from its interior spawn -> it can never leave
                         # the ship into space, whatever the wall geometry does
BOT_RESPOND_RANGE = 1200 # only bots this close to a sabotage bother responding
...
class Bot(pg.sprite.Sprite):
    def __init__(self, game, x, y, bot_direction, bot_type, bot_colour):
        ...
        g = globals()
        self.anim = {
            'Left': g[cl + '_player_imgs_left'],
            'Right': g[cl + '_player_imgs_right'],
            'Up': g[cl + '_player_imgs_up'],
            'Down': g[cl + '_player_imgs_down'],
        }
        self.state = 'wander'        # wander | task | flee | respond
        self.dir = vec(1, 0).rotate(random.randrange(360))
        self.anim_index = 0
        self.last_anim = 0
        self.next_decision = 0
        self.flee_until = 0
        self.detour_until = 0
        self.fixing_since = None
        self.death_time = None       # stamped by the game when this bot dies
        self.reported = False        # this corpse has already been reported
        self.home = vec(x, y)        # spawn point; bots stay leashed near here
    ...
    def update(self):
        # [BOTS] a corpse stays where it fell; frozen during meetings
        ...
        if self.state == 'task':
            ...
            speed = BOT_RUN_SPEED if self.state in ('flee', 'respond') else BOT_SPEED
        ...
        moved = (self.pos - old_pos).length()
        # wedged against a wall: turn back roughly toward home (never grind
        # outward through a gap into space) for a moment
        if self.state != 'task' and moved < BOT_SPEED * self.game.dt * 0.25:
            to_home = self.home - self.pos
            base = to_home.normalize() if to_home.length() > 5 else vec(1, 0).rotate(random.randrange(360))
            self.dir = base.rotate(random.uniform(-75, 75))
            self.detour_until = now + random.randint(300, 700)

        # hard containment backstop: clamp the position onto a circle around the
        # spawn so a bot can NEVER end up outside the ship, no matter how the
        # wall-sliding worked out this frame.
        off = self.pos - self.home
        if off.length() > BOT_HARD_LIMIT:
            self.pos = self.home + off.normalize() * BOT_HARD_LIMIT
            self.rect.x = self.pos.x
            self.rect.y = self.pos.y
            self.dir = (self.home - self.pos)
            self.dir = self.dir.normalize() if self.dir.length() > 1 else vec(1, 0)

        self.animate(now, moved)

    def think(self, now):
        g = self.game

        # fleeing a fresh corpse overrides everything until the timer runs out
        if self.state == 'flee':
        ...
        # assigned responders rush to the sabotage panel and fix it
        if self in getattr(g, 'bot_responders', ()) and g.bot_fix_target is not None:
            to_target = g.bot_fix_target - self.pos
            if to_target.length() <= BOT_FIX_RADIUS:
                if self.state != 'task':
                    self.state = 'task'
                    self.fixing_since = now
            else:
                self.state = 'respond'
                self.fixing_since = None
                if now >= self.detour_until and to_target.length() > 1:
                    self.dir = to_target.normalize()
            return
        if self.state == 'respond' or self.fixing_since is not None:
            # sabotage got fixed (or we were unassigned) while en route
            self.state = 'wander'
            self.fixing_since = None
            self.next_decision = 0

        # containment: if the bot has strayed past its leash, walk it straight
        # back toward its spawn. Spawns are interior points, so this guarantees
        # bots can never wander out of the ship into space.
        off = self.pos - self.home
        if off.length() > BOT_LEASH:
            self.state = 'wander'
            self.dir = (-off).normalize()
            self.detour_until = 0
            self.next_decision = now + random.randint(500, 1000)
            return

        # plain crew behaviour: mill about the local area, pausing at "tasks"
        if now >= self.next_decision and now >= self.detour_until:
            roll = random.random()
            if self.state == 'task':
                self.state = 'wander'
                self.dir = vec(1, 0).rotate(random.randrange(360))
                self.next_decision = now + random.randint(900, 2000)
            elif roll < 0.16:
                self.state = 'task'
                self.next_decision = now + random.randint(1200, 3000)
            else:
                self.state = 'wander'
                # bias new heading slightly back toward home so bots orbit spawn
                if off.length() > BOT_LEASH * 0.6:
                    self.dir = (-off).normalize().rotate(random.uniform(-60, 60))
                else:
                    self.dir = vec(1, 0).rotate(random.randrange(360))
                self.next_decision = now + random.randint(900, 2000)
    ...
    def facing(self):
        if abs(self.dir.x) >= abs(self.dir.y):
            return 'Right' if self.dir.x >= 0 else 'Left'
        return 'Down' if self.dir.y >= 0 else 'Up'

    def animate(self, now, moved):
        frames = self.anim[self.facing()]
        if self.state == 'task' or moved <= 0.5:
            self.anim_index = 0
            self.image = frames[0]
            return
        if now - self.last_anim > 90:
            self.last_anim = now
