...
# [MIDNIGHT] Shadow Protocol fairness layer (fire-and-forget bridge client)
from midnight_hooks import hooks, PLAYERS_PER_MATCH
...
class Game:
    def __init__(self):
        ...
        # [BOTS] freeplay crew AI + clickable HUD action icons
        self.hud_kill_click = False
        self.hud_use_click = False
        self.bot_responders = []
        self.bot_fix_target = None

        # [MIDNIGHT] meeting-anchoring flags: defined here too so freeplay
        # meetings (which share the meeting-close code) never hit an
        # AttributeError — runmultiplayer() re-inits them per match.
        self.mn_eject_seen = False
        self.mn_eject_posted = False
        self.mn_votes_sent = False
        self.night_sync = 0
    ...
    def new(self):
        ...
        hooks.stake_tokens(NIGHT_COIN_ENTRY_STAKE)
        ...
        # [BOTS] fresh AI orchestration state per match
        self.bot_responders = []
        self.bot_fix_target = None
        self.hud_kill_click = False
        self.hud_use_click = False

        # [GUIDE] the 8 completable tasks (finishing all 8 => missions_done == 8
        # => freeplay win). Each carries an `avail` predicate so the on-screen
        # pointer/prompt disappears once that task is done.
        self.task_spots = [
            (vec(5610, 1290), STABILIZE_NAV_RADIUS, "Stabilize Steering",
             lambda: self.stabilize_task_play_count == 1),
            (vec(3940, 321), EMPTY_GARBAGE_RADIUS, "Empty Garbage",
             lambda: self.empty_garbage_task_play_count == 1),
            (vec(3700, 1554), REBOOT_WIFI_RADIUS, "Reboot WiFi",
             lambda: self.reboot_wifi_task_play_count == 1),
            (vec(3166, 1846), FIX_ELECTRICITY_WIRES_RADIUS, "Fix Wiring",
             lambda: self.electricity_wire_task_play_count == 1),
            (vec(1031, 1216), DIVERT_POWER_TOP_REACTOR_RADIUS, "Divert Power",
             lambda: self.divert_power_to_reactor_task_play_count == 1),
            (vec(1117, 837), ALIGN_ENGINE_OUTPUT, "Align Engine",
             lambda: self.align_engine_output_task_play_count == 1),
            (vec(1226, 2300), FUEL_ENGINE, "Fuel Engines",
             lambda: self.fuel_engine_task_play_count == 1),
            (vec(4513, 450), DETECT_RADIUS, "Clear Asteroids",
             lambda: self.clear_asteroid_task_play_count == 1),
        ]
        # utility consoles: usable but not scored, so always promptable
        self.util_spots = [
            (vec(3060, 385), 200, "Admin Console"),
            (vec(1756, 1062), VIEW_SECURITY_MONITOR_RADIUS, "Security Cameras"),
            (vec(3820, 1806), VIEW_ADMIN_MAP_CONTROL_RADIUS, "Admin Map"),
        ]
    ...
    def runmultiplayer(self):
        ...
        # [MIDNIGHT] per-match fairness-layer state
        self.mn_saboteur_id = None      # comes ONLY from the bridge DealResult
        self.mn_deal_applied = False
        self.mn_eject_seen = False      # an ejection happened this meeting
        self.mn_eject_posted = False    # this client posted the ejection itself
        self.mn_votes_sent = False      # local player's vote posted this meeting
        ...
        while self.playing:
                    ...
                    for p in gameEvent:
                        ...
                        # data for our local player, only send it
                            ...
                            if self.eject_sync < p[23] and p[24] != None and (
                                ...
                                # [MIDNIGHT][P4] another client's ejection reached us; its
                                # own client posts the /eject -- just remember it happened
                                self.mn_eject_seen = True
                            ...
                            if self.player.got_votes >= 2 and self.player.alive_status == True and (
                                    self.emerg_meeting_report_status == 1 or self.emerg_meeting_button_status == 1) and self.emergency == True:
                                # [MIDNIGHT][P4] this client is being ejected -- anchor it.
                                # The bridge opens this seat's day-one commitment on-chain,
                                # so the revealed role is provable, not asserted.
                                self.mn_eject_seen = True
                                self.mn_eject_posted = True
                                hooks.eject(self.player.player_id)
                                self.player.alive_status = False
                                self.player.got_reported == True
                            ...
                            # [MIDNIGHT][P1] The base repo's rigged dealer lived here:
                            # "highest player_id becomes imposter". It is DELETED.
                            # Roles now come only from the Midnight bridge (see below,
                            # after the receive loop): gameSeed + role commitments are
                            # anchored on-chain BEFORE any role reaches this client.
                            pass

            # [MIDNIGHT][P1] Fairness-layer deal. Once the lobby holds the full
            # match (PLAYERS_PER_MATCH incl. this client), ask the bridge to deal:
            # it commits gameSeed + role commitments to the Midnight testnet and
            # only then returns saboteurPlayerId. Until the deal is anchored,
            # nobody is the saboteur => the match cannot start unanchored.
            if (not self.mn_deal_applied and self.mn_saboteur_id is None
                    and self.player.player_id != 0
                    and len(self.Players) >= PLAYERS_PER_MATCH):
                hooks.deal_async(
                    list(self.Players.keys()),
                    lambda d: setattr(self, 'mn_saboteur_id', d["saboteurPlayerId"]))
            if self.mn_saboteur_id is not None and not self.mn_deal_applied:
                self.mn_deal_applied = True
                self.player.imposter = (self.player.player_id == self.mn_saboteur_id)
                hooks.commit(list(self.Players.keys()))
            ...
            if len(self.Players) > 1:
                ...
                for p in self.Players.values():
                    ...
                    for m in self.ambient_sounds.values():
                        m.stop()
                    # [MIDNIGHT][P5] agents win by tasks (task wins are attested by
                    # the game; the contract cannot see tasks -- documented boundary)
                    if not self.mn_gameover_sent:
                        self.mn_gameover_sent = True
                        hooks.gameover(1, "tasks")
                        if not self.player.imposter:
                            hooks.reward_tokens(NIGHT_COIN_WIN_REWARD)
                    self.effect_sounds["victory_crew"].play()
                    self.menu.game_over(self.score_list, '')
                    return
                ...
                for p in self.Players.values():
                    if p.alive_status == False and p.imposter == True and self.emergency == False:
                        ...
                        for m in self.ambient_sounds.values():
                            m.stop()
                        # [MIDNIGHT][P5] agents win: the saboteur was ejected
                        if not self.mn_gameover_sent:
                            self.mn_gameover_sent = True
                            hooks.gameover(1, "ejection")
                            if not self.player.imposter:
                                hooks.reward_tokens(NIGHT_COIN_WIN_REWARD)
                        self.effect_sounds["victory_crew"].play()
                        # self.effect_sounds["victory_imposter"].play()
                ...
                for p in self.Players.values():
                    ...
                    if self.emergency == False and self.kill_victim_anim == False:
                        ...
                        for m in self.ambient_sounds.values():
                            m.stop()
                        # [MIDNIGHT][P5] saboteur wins: all agents dead
                        if not self.mn_gameover_sent:
                            self.mn_gameover_sent = True
                            hooks.gameover(2, "kills")
                ...
                if self.sabotagecritical == True and (
                    ...
                    for m in self.ambient_sounds.values():
                        m.stop()
                    # [MIDNIGHT][P5] saboteur wins: critical sabotage timer expired
                    if not self.mn_gameover_sent:
                        self.mn_gameover_sent = True
                        hooks.gameover(2, "sabotage")
                        if self.player.imposter:
                            hooks.reward_tokens(NIGHT_COIN_WIN_REWARD)
                    self.effect_sounds["victory_imposter"].play()
                    self.menu.game_over_imposter(self.score_list, '')
    ...
    def update(self):
        # [BOTS] freeplay crew AI orchestration (flee / report / fix sabotage)
        ...
        for hit in hits:
            if hit.type == 'vent':
                ...
                if keys[pg.K_SPACE] or keys[pg.K_e] or self.hud_use_click:
        ...
        # [GUIDE] player reports a nearby corpse in freeplay (press R). Opens the
        # same dead-body-report meeting the bots use.
        if self.gamemode == "Freeplay" and self.player.alive_status and not self.emergency:
            rkeys = pg.key.get_pressed()
            if rkeys[pg.K_r]:
                for corpse in self.bots:
                    if (not corpse.alive_status and not corpse.reported
                            and corpse.death_time is not None
                            and (corpse.pos - self.player.pos).length() < 160):
                        self.player_report_body(corpse)
                        break
        ...
        for hit in hitz:
            if hit.type == 'bot1':
                keys = pg.key.get_pressed()
                if keys[pg.K_RETURN] or self.hud_kill_click:
        ...
        for hit in hitp:
            ...
            if keys[pg.K_RETURN] or self.hud_kill_click:
                if (self.killcooldown - self.killcooldown_start) > 15000 and hit.alive_status == True and self.player.imposter == True and self.invisible_play_count == 0 and self.emergency == False:
                    ...
                    # [MIDNIGHT][P2] anchor the kill: the bridge proves in ZK that
                    # this client's committed role is SABOTEUR, without recording
                    # the killer's seat on-chain. Fire-and-forget, never blocks.
                    hooks.kill(self.player.player_id, hit.player_id)
    ...
    # [BOTS] ---- freeplay crew AI orchestration -------------------------------
    # Bots wander on their own (sprites.Bot). This method handles everything
    # that needs game state: panic when a body drops, assigning responders to
    # an active sabotage, completing the fix, and reporting found corpses.
    def update_bot_ai(self):
        now = pg.time.get_ticks()

        # 1) a bot just died: stamp the corpse, nearby crew scatter in panic
        for b in self.bots:
            if not b.alive_status and b.death_time is None:
                b.death_time = now
                for w in self.bots:
                    if w.alive_status and (w.pos - b.pos).length() < 380:
                        w.state = 'flee'
                        w.flee_until = now + random.randint(1600, 2600)
                        away = w.pos - b.pos
                        w.dir = away.normalize() if away.length() > 0 else vec(1, 0).rotate(random.randrange(360))

        # 2) active sabotage: the two closest living bots run to the panel
        if self.night_reactor:
            target = vec(889, 999)      # reactor (same spot the player fixes)
        elif self.night:
            target = vec(2472, 1721)    # lights panel
        else:
            target = None
        ...
        if target is None:
            ...
            self.bot_fix_target = target
            self.bot_responders = [b for b in self.bot_responders if b.alive_status]
            if len(self.bot_responders) < 2:
                # only bots already near the panel respond, so nobody treks
                # across the whole ship (and risks wall-wedging on the way)
                candidates = [b for b in self.bots
                              if b.alive_status and b not in self.bot_responders
                              and (b.pos - target).length() < BOT_RESPOND_RANGE]
                candidates.sort(key=lambda b: (b.pos - target).length())
                self.bot_responders += candidates[:2 - len(self.bot_responders)]
            # a responder that has stood at the panel long enough completes the fix
            for b in self.bot_responders:
                if b.state == 'task' and b.fixing_since is not None and now - b.fixing_since > 2800:
                    if self.night_reactor:
                        self.bot_fix_reactor()
                    else:
                        self.bot_fix_lights()
                    b.fixing_since = None
                    self.bot_responders = []
        ...
        # 3) corpse discovery -> a crewmate reports the body (opens the meeting)
        if not (self.emergency or self.emerg_meeting_button_status
                or self.emerg_meeting_report_status or self.sabotagecritical):
            sight = 170 if self.night else 300
            for corpse in self.bots:
                if corpse.alive_status or corpse.reported or corpse.death_time is None \
                        or now - corpse.death_time < 1500:
                    continue
                for w in self.bots:
                    if w.alive_status and w.state != 'flee' and (w.pos - corpse.pos).length() < sight:
                        self.bot_report_body(w, corpse)
                        return

    def _open_report_meeting(self, img_name):
        # shared dead-body-report meeting opener (bot- or player-triggered)
        self.emergency_img_sync_report = img_name
        self.effect_sounds['emergency_alarm'].play()
        self.emerg_meeting_report_status = 1
        self.emergency = True
        self.emergency_meeting_index = 0
    ...
    def bot_report_body(self, reporter, corpse):
        corpse.reported = True
        img_name = reporter.bot_colour.lower() + '_player_emergency_meeting_report'
        if img_name not in globals():
            # only the 5 main colours ship a report alert image
            img_name = 'red_player_emergency_meeting_report'
        self._open_report_meeting(img_name)

    def player_report_body(self, corpse):
        corpse.reported = True
        self._open_report_meeting(self.player.emergency_meeting_img_sync_report)

    # [GUIDE] ---- freeplay on-screen guidance (prompts + sabotage arrows) ------
    def nearest_task_spot(self):
        # nearest task that is still incomplete (done tasks drop off the pointer)
        p = self.player.pos
        best, bestd = None, 1e9
        for (pos, radius, name, avail) in getattr(self, 'task_spots', []):
            if not avail():
                continue
            d = (pos - p).length()
            if d < bestd:
                bestd, best = d, (pos, radius, name)
        return best

    def get_interaction_prompt(self):
        # returns (key_label, action_label) for the highest-priority thing the
        # player can do right where they're standing, else None
        p = self.player.pos
        for b in self.bots:
            if (not b.alive_status and b.death_time is not None and not b.reported
                    and (b.pos - p).length() < 160):
                return ("R", "Report Body")
        hits = pg.sprite.spritecollide(self.player, self.items, False)
        for hit in hits:
            if hit.type == 'emerg_btn':
                return ("SPACE", "Emergency Meeting")
        if self.player.imposter:
            for b in self.bots:
                if b.alive_status and (b.pos - p).length() < 120:
                    return ("ENTER", "Kill")
        for hit in hits:
            if hit.type == 'vent' and self.player.imposter:
                return ("E", "Exit Vent" if self.invisible_play_count else "Vent")
        for (pos, radius, name, avail) in getattr(self, 'task_spots', []):
            if avail() and (pos - p).length() <= radius:
                return ("E", name)
        for (pos, radius, name) in getattr(self, 'util_spots', []):
            if (pos - p).length() <= radius:
                return ("E", name)
        return None

    def draw_freeplay_guides(self):
        if self.gamemode != "Freeplay" or not self.player.alive_status:
            return
        if (self.paused or self.emergency or self.emerg_meeting_button_status
                or self.emerg_meeting_report_status or self.isdoingTask
                or self.mini_map_button_status or self.clear_asteroid_task_window_status):
            return
        self.draw_sabotage_arrow()
        prompt = self.get_interaction_prompt()
        if prompt:
            self.draw_interaction_prompt(prompt[0], prompt[1])
        elif self.invisible_play_count == 0:
            nt = self.nearest_task_spot()
            if nt is not None:
                self.draw_task_pointer(nt[0], nt[2])

    def draw_interaction_prompt(self, key, label):
        font = pg.font.Font(FONT, 20)
        text = font.render("[ " + key + " ]   " + label, True, WHITE)
        pad = 16
        w, h = text.get_width() + pad * 2, text.get_height() + pad
        x, y = WIDTH // 2 - w // 2, HEIGHT - 155
        box = pg.Surface((w, h), pg.SRCALPHA)
        box.fill((0, 0, 0, 175))
        pg.draw.rect(box, (255, 214, 10), box.get_rect(), 2, border_radius=8)
        self.screen.blit(box, (x, y))
        self.screen.blit(text, (x + pad, y + pad // 2))

    def draw_task_pointer(self, target, name):
        d = vec(target) - self.player.pos
        if d.length() < 8:
            return
        dirn = d.normalize()
        center = vec(WIDTH / 2, HEIGHT / 2)
        tip = center + dirn * 155
        perp = vec(-dirn.y, dirn.x)
        pg.draw.polygon(self.screen, (120, 200, 255),
                        [tip, tip - dirn * 24 + perp * 13, tip - dirn * 24 - perp * 13])
        font = pg.font.Font(FONT, 14)
        label = font.render(name, True, (205, 230, 255))
        lp = tip - dirn * 44
        self.screen.blit(label, (lp.x - label.get_width() / 2, lp.y - label.get_height() / 2))

    def draw_sabotage_arrow(self):
        if self.night:
            target, label, col = vec(2472, 1721), "LIGHTS SABOTAGED", (255, 214, 10)
        elif self.night_reactor or self.sabotagecritical:
            target, label, col = vec(889, 999), "REACTOR MELTDOWN", (255, 90, 90)
        else:
            return
        d = vec(target) - self.player.pos
        if d.length() <= DETECT_RADIUS_SABOTAGE_FIX:
            return
        dirn = d.normalize()
        center = vec(WIDTH / 2, HEIGHT / 2 - 95)
        tip = center + dirn * 46
        perp = vec(-dirn.y, dirn.x)
        pg.draw.polygon(self.screen, col,
                        [tip, center - dirn * 6 + perp * 17, center - dirn * 6 - perp * 17])
        font = pg.font.Font(FONT, 18)
        text = font.render(label, True, col)
        self.screen.blit(text, (WIDTH / 2 - text.get_width() / 2, HEIGHT / 2 - 145))

    def bot_fix_lights(self):
        # mirrors the player's K_CTRL lights fix
        self.night = False
        self.time_left_to_light = 15
        pygame.time.set_timer(self.light_timer_event, 1000)
        self.light_bulb_timer_icon_status = True
    ...
    def bot_fix_reactor(self):
        # mirrors the player's K_SHIFT reactor fix
        self.night_reactor = False
        self.time_left_to_boom_cooldown = 15
        pg.time.set_timer(self.reactor_timer_cooldown_event, 1000)
        self.reactor_timer_cooldown_visible_status = True
    ...
    def freeplay_meeting_reset(self):
        # after a freeplay meeting: clear reported corpses, scatter live bots
        if self.gamemode != "Freeplay":
            return
        now = pg.time.get_ticks()
        for b in list(self.bots):
            if not b.alive_status:
                if b.reported:
                    b.kill()
                continue
            b.pos = vec(random.choice(self.player_pos))
            b.state = 'wander'
            b.flee_until = 0
            b.detour_until = 0
            b.fixing_since = None
            b.next_decision = now + random.randint(300, 1200)
    ...
    def draw(self):
        ...
        if self.emerg_meeting_button_status:
            ...
            if self.emergency_meeting_index > 2:
                ...
                # [MIDNIGHT][P4] meeting closed. If nobody was ejected, anchor the
                # no-ejection meeting end (every client posts; the bridge keeps the
                # first per round and rejects the rest). If an ejection was posted
                # by the ejected client, just advance the local round counter.
                if self.gamemode == "Multiplayer":
                    if not self.mn_eject_seen:
                        hooks.eject(None)
                    elif not self.mn_eject_posted:
                        hooks.round += 1
                self.mn_eject_seen = False
                self.mn_eject_posted = False
                self.mn_votes_sent = False
        ...
        if self.emerg_meeting_report_status:
            ...
            if self.emergency_meeting_index > 2:
                ...
                # [MIDNIGHT][P4] meeting closed (dead-body-report flow) -- same
                # no-ejection anchoring as the emergency-button flow above.
                if self.gamemode == "Multiplayer":
                    if not self.mn_eject_seen:
                        hooks.eject(None)
                    elif not self.mn_eject_posted:
        ...
        for hit in hits:
            if hit.type == 'emerg_btn' and not self.mini_map_button_status and self.emerg_meeting_button_status == 0 and self.sabotagecritical == False and self.player.alive_status == True and self.emergency == False:
                keys = pg.key.get_pressed()
                if keys[pg.K_SPACE] or self.hud_use_click:
        ...
        if self.gamemode == "Freeplay" and (self.task_button_show_status or self.player.imposter):
        ...
        if d.distance_to(c) <= 200 and self.gamemode == "Freeplay":
            if (keys[pg.K_SPACE] or keys[pg.K_e] or self.hud_use_click) and self.open_cafe_comp_sound_play_count == 1:
        ...
        if x.distance_to(y) <= STABILIZE_NAV_RADIUS:
            if (keys[pg.K_SPACE] or keys[pg.K_e] or self.hud_use_click) and self.stablize_sound_play_count == 1 and self.stabilize_task_play_count == 1 and (not self.player.imposter or self.gamemode == "Freeplay"):
                self.effect_sounds['selected'].play()
        ...
        if x.distance_to(y) <= EMPTY_GARBAGE_RADIUS:
            if (keys[pg.K_SPACE] or keys[pg.K_e] or self.hud_use_click) and self.empty_garbage_sound_play_count == 1 and self.empty_garbage_task_play_count == 1 and (not self.player.imposter or self.gamemode == "Freeplay"):
                self.effect_sounds['selected'].play()
        ...
        if x.distance_to(y) <= REBOOT_WIFI_RADIUS:
            if (keys[pg.K_SPACE] or keys[pg.K_e] or self.hud_use_click) and self.reboot_wifi_sound_play_count == 1 and self.reboot_wifi_task_play_count == 1 and (not self.player.imposter or self.gamemode == "Freeplay"):
                self.effect_sounds['selected'].play()
        ...
        if x.distance_to(y) <= FIX_ELECTRICITY_WIRES_RADIUS:
            if (keys[pg.K_SPACE] or keys[pg.K_e] or self.hud_use_click) and self.electricity_wire_sound_play_count == 1 and self.electricity_wire_task_play_count == 1 and (not self.player.imposter or self.gamemode == "Freeplay"):
                self.effect_sounds['selected'].play()
        ...
        if x.distance_to(y) <= DIVERT_POWER_TOP_REACTOR_RADIUS:
            if (keys[pg.K_SPACE] or keys[pg.K_e] or self.hud_use_click) and self.divert_power_to_reactor_sound_play_count == 1 and self.divert_power_to_reactor_task_play_count == 1 and (not self.player.imposter or self.gamemode == "Freeplay"):
                self.effect_sounds['selected'].play()
                self.effect_sounds['fix_electric_wires_BG'].play(-1)
        ...
        if c.distance_to(d) <= ALIGN_ENGINE_OUTPUT:
            if (keys[pg.K_SPACE] or keys[pg.K_e] or self.hud_use_click) and self.align_engine_output_task_play_count ==1 and self.align_engine_output_sound_play_count == 1 and (not self.player.imposter or self.gamemode == "Freeplay"):
                self.effect_sounds['selected'].play()
        ...
        if n.distance_to(m) <= PICK_STORAGE_GAS_CAN_RADIUS:
            if (keys[pg.K_SPACE] or keys[pg.K_e] or self.hud_use_click) and self.fuel_engine_task_play_count == 1 and self.gas_can_picking_count == 1 and self.gas_can_picking_sound_play_count == 1 and (self.player.imposter == False or self.gamemode == "Freeplay"):
        ...
        if c.distance_to(d) <= FUEL_ENGINE:
            if (keys[pg.K_SPACE] or keys[pg.K_e] or self.hud_use_click) and self.fuel_engine_sound_play_count == 1 and self.fuel_engine_task_play_count == 1 and (not self.player.imposter or self.gamemode == "Freeplay"):
            ...
            if self.clear_asteroid_task_available:
                ...
                for i in range(self.num_of_asteroids):
                    ...
                    if self.asteroid_kill_count == 30:
                        ...
                        if not self.player.imposter or self.gamemode == "Freeplay":
                            self.clear_asteroid_task_play_count -= 1
        ...
        if d.distance_to(c) <= DETECT_RADIUS and self.clear_asteroid_sound_play_count == 1 and self.clear_asteroid_task_play_count == 1:
            if self.player.imposter == False or self.gamemode == "Freeplay":
                if keys[pg.K_SPACE] or keys[pg.K_e] or self.hud_use_click:
        ...
        # Night Coin HUD display
        if not self.emerg_meeting_button_status and not self.eject:
            coin_hud = self.board.draw_night_coin_hud(hooks.token_balance, NIGHT_COLOR_COIN, 22)
            self.screen.blit(coin_hud, (WIDTH - coin_hud.get_width() - 20, 20))
        ...
        # [GUIDE] freeplay on-screen prompts + sabotage direction arrow
        self.draw_freeplay_guides()
        ...
        if (x.distance_to(z) <= VIEW_SECURITY_MONITOR_RADIUS and not self.mini_map_button_status) or (y.distance_to(z) <= VIEW_ADMIN_MAP_CONTROL_RADIUS and not self.mini_map_button_status):
            if (keys[pg.K_SPACE] or keys[pg.K_e] or self.hud_use_click) and self.view_admin_security_monitor_sound_play_count == 1 and self.player.alive_status:
                self.effect_sounds['selected'].play()
    ...
    def events(self):
        # [BOTS/UX] one-frame flags set by clicking the HUD action icons;
        # consumed by update()/draw() later this same frame
        self.hud_kill_click = False
        self.hud_use_click = False
        ...
        for event in pg.event.get():
            ...
            # [BOTS/UX] HUD action icons are clickable: KILL / SABOTAGE /
            # LIGHTS / EMERGENCY behave exactly like their hotkeys
            # (Enter / Shift / Ctrl / Space).
            if event.type == pg.MOUSEBUTTONDOWN and event.button == LEFT_MOUSE_BUTTON \
                    and not self.paused and not self.emergency and self.player.alive_status:
                mx, my = event.pos
                kill_r = pg.Rect(WIDTH - 400, HEIGHT - 110,
                                 self.kill_icon.get_width(), self.kill_icon.get_height())
                sab_r = pg.Rect(WIDTH - 280, HEIGHT - 110,
                                self.sabotage_icon.get_width(), self.sabotage_icon.get_height())
                light_r = pg.Rect(WIDTH - 160, HEIGHT - 117,
                                  self.light_bulb_icon.get_width(), self.light_bulb_icon.get_height())
                emerg_r = pg.Rect(WIDTH - 540, HEIGHT - 109,
                                  self.emergency_icon.get_width(), self.emergency_icon.get_height())
                if kill_r.collidepoint(mx, my):
                    self.hud_kill_click = True
                    # audible feedback when there is no target in range
                    if self.player.imposter \
                            and not pg.sprite.spritecollide(self.player, self.bots, False) \
                            and not pg.sprite.spritecollide(self.player, self.players_server, False):
                        self.effect_sounds['imposter_kill_cooldown_sound'].play()
                elif sab_r.collidepoint(mx, my):
                    pg.event.post(pg.event.Event(pg.KEYDOWN, key=pg.K_LSHIFT, mod=0, unicode=''))
                elif light_r.collidepoint(mx, my):
                    pg.event.post(pg.event.Event(pg.KEYDOWN, key=pg.K_LCTRL, mod=0, unicode=''))
                elif emerg_r.collidepoint(mx, my):
                    self.hud_use_click = True
            ...
            if event.type == pg.MOUSEBUTTONDOWN and event.button == LEFT_MOUSE_BUTTON and not self.paused and (
                ...
                # [MIDNIGHT][P3] post the local player's vote once per meeting.
                # Votes are stored as colours in this repo; map colour -> player_id.
                # The bridge enforces one-vote-per-seat-per-meeting with a ZK
                # nullifier on-chain (and 409s any duplicate as the belt).
                if self.player.voted is not None and not self.mn_votes_sent \
                        and self.gamemode == "Multiplayer" and hasattr(self, 'Players'):
                    self.mn_votes_sent = True
                    mn_target_id = None
                    for mn_p in self.Players.values():
                        if mn_p.player_colour == self.player.voted:
                            mn_target_id = mn_p.player_id
                            break
                    hooks.vote(self.player.player_id, mn_target_id)
